...
export function SBSExerciseCard({
  ...
  // Animation effect - runs after state updates via updateSetsCompleted
  useEffect(() => {
    return () => {
  ...
  const updateSetsCompleted = (value: number) => {
    ...
    const prevValue = prevSetsRef.current;
    ...
    if (newValue >= setGoalMin && prevValue < setGoalMin) {
      setShowSuccess(true);
      setTimeout(() => setShowSuccess(false), 600);
    }

    prevSetsRef.current = newValue;
