...
export function RegularExerciseCard({
  ...
  useEffect(() => {
    const loadPreviousData = async () => {
      ...
      if (data) {
        ...
        // Only set defaults on initial load, not when inputs change
        if (data.sets.length > 0) {
          setNewSetWeight((prev) => prev || data.sets[0].weight.toString());
          setNewSetReps((prev) => prev || data.sets[0].reps.toString());
        }
      }
    };
    loadPreviousData();
  ...
  // Helper to trigger success animation when target is reached
  const checkAndTriggerSuccess = (newCompletedCount: number) => {
    const prevCount = prevSetsCountRef.current;
    if (targetSets && newCompletedCount >= targetSets && prevCount < targetSets) {
      setShowSuccess(true);
      setTimeout(() => setShowSuccess(false), 600);
    }
    prevSetsCountRef.current = newCompletedCount;
  };
  ...
  const addSet = () => {
    ...
    // Check for success animation (new set is completed, so add 1)
    const newCompletedCount = completedSets + 1;
    checkAndTriggerSuccess(newCompletedCount);
